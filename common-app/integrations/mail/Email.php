...
class Email extends \QModel
{
    ...
    private $headersStr;
    private $attachments;
    private $contentTypeStr = "text/plain";
	private $reply_to = [];
	private $from_alias = null;
	...
	private function _sendAuthMail()
	{
			...
			if (!$mailSender)
			{
				...
				while ((!$mailSender) && $current_owner)
				{
					...
					if (!$in_partner && $in_partner[0] && $in_partner[0]->Owner)
						break;
					$partner_id = $in_partner[0]->Owner->Id;
					...
					else
					{
						...
						if ($mailSender_list && $mailSender_list[0])
							$mailSender = $mailSender_list[0];

						$current_owner = \QQuery("Companies.{* WHERE Id=?}", $partner_id)->Companies;
						$current_owner = $current_owner ? $current_owner[0] : null;
					}
				}
			}
		}
