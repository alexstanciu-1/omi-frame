...
trait QModel_Methods
{
	...
	public static function GetTypeByName($type)
	{
		...
		else if ($type[0] === strtolower($type[0]))
		{
			...
			switch ($type)
			{
				...
				case "integer":
				case "int":
					return self::$_Types_Cache[$type] = QModelTypeInteger::GetType();
				case QModelType::ModelTypeString:
				case "string":
					return self::$_Types_Cache[$type] = QModelTypeString::GetType();
				case QModelType::ModelTypeFloat:
				case "float":
				case "double":
					return self::$_Types_Cache[$type] = QModelTypeFloat::GetType();
				case QModelType::ModelTypeBoolean:
				case "boolean":
				case "bool":
					return self::$_Types_Cache[$type] = QModelTypeBoolean::GetType();
				case QModelType::ModelTypeNull:
				case "null":
					return self::$_Types_Cache[$type] = QModelTypeNull::GetType();
				case QModelType::ModelTypeArray:
				case "array":
					return self::$_Types_Cache[$type] = QModelTypeArray::GetType();
				case QModelType::ModelTypeDate:
				case "date":
					return self::$_Types_Cache[$type] = QModelTypeDate::GetType();
				case QModelType::ModelTypeDateTime:
				case "datetime":
					return self::$_Types_Cache[$type] = QModelTypeDateTime::GetType();
				case QModelType::ModelTypeTime:
				case "time":
					return self::$_Types_Cache[$type] = QModelTypeTime::GetType();
				case QModelType::ModelTypeTimestamp:
				case "timestamp":
					return self::$_Types_Cache[$type] = QModelTypeTimestamp::GetType();
				case QModelType::ModelTypeResource:
				case "resource":
					return self::$_Types_Cache[$type] = QModelTypeResource::GetType();
				case QModelType::ModelTypeMixed:
				case "mixed":
					return self::$_Types_Cache[$type] = QModelTypeMixed::GetType();
				case QModelType::ModelTypeCallback:
				case "callback":
					return self::$_Types_Cache[$type] = QModelTypeCallback::GetType();
				case QModelType::ModelTypeScalar:
				case "scalar":
					return self::$_Types_Cache[$type] = QModelTypeScalar::GetType();
				case QModelType::ModelTypeScalarOrArray:
				case "scalarorarray":
					return self::$_Types_Cache[$type] = QModelTypeScalarOrArray::GetType();
				case QModelType::ModelTypeObject:
				case "object":
					return self::$_Types_Cache[$type] = QModelTypeObject::GetType();
				case QModelType::ModelTypeFile:
				case "file":
					return self::$_Types_Cache[$type] = QModelTypeFile::GetType();
				default:
				{
		...
		else
		{
			...
			if (!file_exists($cache_file))
			{
				...
				return (self::$_Types_Cache[$type] = (is_array($cache_res) ? reset($cache_res) : $cache_res));
			}
			else
			{
				...
				return (self::$_Types_Cache[$type] = ${"Q_TYPECACHE_{$str_type}"});
			}
		}
	}
	...
	public function current () { /* @TODO - maybe use the getter! */ return $this->{static::$_Type_Properties[static::class][$this->_iter]}; }
	
	public function key () { return static::$_Type_Properties[static::class][$this->_iter]; }
	
	public function next () { $this->_iter++; }
	
	public function rewind () {
		
		if (!isset(static::$_Type_Properties[static::class]))
		{
			if (($del = ($props = $this->getModelType()->properties)["Del__"]))
				unset($props["Del__"]);
			static::$_Type_Properties[static::class] = $props->getKeys();
