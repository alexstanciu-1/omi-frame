<tr xg-security="$grid_mode, $settings['model:property'], $vars_path, $data" xg-item='<?= $xg_tag ?>' class="item qc-xg-item qc-ref-ctrl-form"
	...
	<?php $hide_pickers = ($config && $config['__settings__'] && $config['__settings__']['bulk'] && $config['__settings__']['bulk']['hide_pickers']);  ?>
	@if (false)
	...
	<?php endif; if ($properties && (count($properties) > 0)) : 
					...
					<ul class='dropdown bg-white'>
						...
						@if ($can_delete)
							<li xg-security="'delete', $settings['model:property'], $vars_path, $data" class="q-delete-action-wr">
								<a title='{{_L("Delete")}}' 
								   class="delete nowrap pointer qc-ref-ctrl-delete-flag qc-tooltip tooltip-left">
