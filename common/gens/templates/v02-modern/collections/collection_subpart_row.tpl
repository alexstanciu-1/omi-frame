		...
		<td class='qc-actions-wr'>
			@if (!$_hide_rm)
				<ul class="actions mt-1 mb-3">
					<li>
						<a href="javascript: void(0);" title="{{_L('Delete')}}" class="inline-flex items-center px-4 py-2 border border-red-300 text-sm leading-5 rounded-md text-red-700 bg-white hover:text-red-500 focus:outline-none focus:shadow-outline-indigo focus:border-indigo-300 active:text-indigo-800 active:bg-indigo-50 active:text-indigo-800 transition duration-150 ease-in-out font-medium qc-ref-ctrl-delete qc-delete" {{$_qengine_args['_tsp'] ? " data-vars-path='".$_qengine_args['_tsp'][<?= $isOneToMany ? '1' : '0' ?>]."'" : ""}}>
							{{_L('Delete')}}
						</a>
					</li>
				</ul>
