...
$_Q_Validators = [
	...
	"mandatory" => ["(!empty(\$value) || (\$value === \"0\") || (\$value === 0))", "<b>{{_L('\$property')}}</b> {{_L('is mandatory')}}"],
	...
	"email" => ["filter_var(\$value, FILTER_VALIDATE_EMAIL) !== false", "Invalid email <b>\$value</b>!", "Email correct format is nicknamedomain.com"],
	"ip" => ["filter_var(\$value, FILTER_VALIDATE_IP) !== false", "Invalid IP \$value", "IP correct format is xxx.xxx.xxx.xxx"],
	"mac" => ["filter_var(\$value, FILTER_VALIDATE_MAC) !== false", "Invalid MAC \$value", "MAC address correct format is xx.xx.xx.xx.xx.xx"],
	"float" => ["filter_var(\$value, FILTER_VALIDATE_FLOAT) !== false", "Only numeric values are allowed", "Only numeric values are allowed"],
	"int" => ["filter_var(\$value, FILTER_VALIDATE_INT) !== false", "Only integer values are allowed", "Only integer values are allowed"],
	"validDateYMD" => ["qIsValidDate(\$value)", "Date \$value is not valid (Valid format is YYYY-mm-dd)"],
	"validHour" => ["qIsValidHour(\$value)", "Hour \$value is not valid (Valid format is HH:ii:ss)"],
